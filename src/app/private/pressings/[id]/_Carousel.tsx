...
export default function Carousel({ pressing }: any) {
    const [thumbsSwiper, setThumbsSwiper] = useState<SwiperType | null>(null);

    if (pressing.pictures.length === 0) {
        return (
    ...
    return (
        <div className="w-full">
            ...
            <div className="relative mb-4">
                ...
                >
                    {pressing.pictures.map((picture: any, index: number) => (
                        <SwiperSlide key={index}>
                            <div className="w-full h-full relative">
                                <img
                                    src={picture.data_url}
                                    alt={`${pressing.name} - photo ${index + 1}`}
                        ...
                        <SwiperSlide key={index} className="cursor-pointer">
                            <div className="w-full h-full relative overflow-hidden rounded-md border-2 border-transparent hover:border-blue-500 transition-all duration-200">
                                <img
                                    ...
                                    alt={`${pressing.name} - miniature ${index + 1}`}
